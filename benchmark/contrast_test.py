...
def run_optimized_conversation(openai_client: OpenAI) -> list[TurnResult]:
    ...
    Key optimization: refresh_memory() is called BEFORE each chat() so that
    the session_memory layer (Layer 4) is determined before the message
    sequence is built. This avoids prefix breakage from mid-request memory
    updates. The stable prefix (Layers 1-3) remains intact, and the
    session_memory layer stays consistent within each request.

    History is manually truncated after memory compression to keep the
    prompt size from growing unboundedly, which would dilute the hit rate.
    """
    runtime = CacheAwareClient(
        chat_callable=lambda messages, **kwargs: openai_client.chat.completions.create(
            model=MODEL, messages=messages, **kwargs,
        ...
        metrics=CacheMetrics.from_provider(MODEL),
        memory=SessionMemory(max_summary_chars=300, max_keywords=8),
    )
    ...
    results: list[TurnResult] = []
    for turn in range(1, TURNS + 1):
        # Compress history into session memory BEFORE building the request,
        # so the memory layer is stable within this turn's message sequence.
        # Skip turn 1 (no history to compress yet).
        if turn >= 3:
            runtime.refresh_memory()
            # Truncate history after compression — the summary in session_memory
            # preserves the context, so older turns are no longer needed.
            # Keep only the last 2 turns in history to limit prompt growth.
            if len(runtime.history) > 4:
                runtime.history = runtime.history[-4:]
...
HITRATE_CHART_PATH = OUTPUT_DIR / "contrast_hit_rate.png"
COST_CHART_PATH = OUTPUT_DIR / "contrast_cost.png"


def write_chart(scenarios: list[ScenarioResult]) -> None:
    """Generate hit-rate and cost comparison charts."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt

    colours = {"naive": "#e74c3c", "optimized": "#2ecc71"}
    scenario_names = [s.name for s in scenarios]

    # ── Chart 1: Hit rate per turn (3 subplots) ──
    fig, axes = plt.subplots(1, 3, figsize=(15, 4.5), sharey=False)
    fig.suptitle("Cache Hit Rate by Turn — Naive vs Optimized", fontsize=13, fontweight="bold")

    for ax, s in zip(axes, scenarios):
        turns = [r.turn for r in s.naive_results]
        ax.plot(turns, [r.hit_rate * 100 for r in s.naive_results],
                marker="o", color=colours["naive"], label="Naive", linewidth=2)
        ax.plot(turns, [r.hit_rate * 100 for r in s.optimized_results],
                marker="s", color=colours["optimized"], label="Optimized", linewidth=2)
    ...
    fig.savefig(HITRATE_CHART_PATH, dpi=150)
    plt.close(fig)

    # ── Chart 2: Total cost per scenario (grouped bar) ──
    fig, ax = plt.subplots(figsize=(8, 5))
    x = range(len(scenario_names))
    width = 0.35

    naive_costs = [s.naive_total_cost for s in scenarios]
    opt_costs = [s.optimized_total_cost for s in scenarios]

    bars1 = ax.bar([i - width / 2 for i in x], naive_costs, width,
                   label="Naive", color=colours["naive"], alpha=0.85)
    bars2 = ax.bar([i + width / 2 for i in x], opt_costs, width,
                   label="Optimized", color=colours["optimized"], alpha=0.85)

    ax.set_ylabel("Total Cost (USD)")
    ax.set_title("Total Input Cost by Scenario — Naive vs Optimized", fontweight="bold")
    ...
    plt.tight_layout()
    fig.savefig(COST_CHART_PATH, dpi=150)
    plt.close(fig)
